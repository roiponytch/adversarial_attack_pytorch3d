...
# # add path for demo utils functions
#
# # sys.path.insert(0, '/media/adversarial/pytorch3d_/pytorch3d-master')
# # from docs.tutorials.utils.plot_image_grid import image_grid
...
# Setup
# Set paths
# DATA_DIR = "/media/adversarial/blender_files/"
# obj_path = "/media/adversarial/blender_files/Imerkava_Interactive_pony.obj"
...
# Initialize a camera.adv_test_batch['img'] = [data_batch['img'][0].clone()]
...
# R, T = look_at_view_transform(2, 140, 90) 
...
# lights = PointLights(device=device,diffuse_color=[[1., 1., 1.]],ambient_color=[[1, 1, 1],],specular_color=((0.1, .1, .1),) ,location=[[0, 10, 0]])

# lights = PointLights(device=device ,location=[[10, 10, 0]])
...
if 0:
    ...
    # With world coordinates +Y up, +X left and +Z in, the front of the cow is facing the -Z direction. 
    # So we move the camera by 180 in the azimuth direction so it is facing the front of the cow.
    elev = torch.linspace(10, 10, num_views_total)
    azim = torch.linspace(100, 240, num_views_total)
    dist = torch.linspace(2, 4, num_views_total)
    ...
    # Define the settings for rasterization and shading. Here we set the output image to be of size
    # 512x512. As we are rendering images for visualization purposes only we will set faces_per_pixel=1
    # andplt.imread() blur_radius=0.0. We also set bin_size and max_faces_per_bin to None which ensure that
    # the fas ter coarse-to-fine rasterization method is used. Refer to rasterize_meshes.py for 
    # explanations of these parameters. Refer to docs/notes/renderer.md for an explanation of 
    # the difference between naive and coarse-to-fine rasterization. 
    raster_settings = RasterizationSettings(
...
#%%
if 0:
    delta_1 = torch.nn.Parameter( torch.zeros_like(meshes.textures.maps_padded()[0],requires_grad=True, device=device))
    # delta_1 = torch.zeros_like(meshes.textures.maps_padded()[0])
    ones_mask = torch.ones_like(meshes.textures.maps_padded())
    # delta_1.requires_grad =True
    delta_1.retain_grad()
...
#%%
if 0:
    cb =chess_board(shape=(2048,2048,3), divs=12)
    mask = torch.from_numpy(cb[np.newaxis]).type(mesh.textures._maps_padded.dtype).to(device)
    delta_1 = torch.nn.Parameter( torch.zeros((3),requires_grad=True, device=device))
    delta_2 = torch.nn.Parameter( torch.zeros((3),requires_grad=True, device=device))
...
if 0:
    ...
    cb =n_chess_board(shape=(2048,2048,3), divs=8)
    # mask = torch.from_numpy(cb).type(mesh.textures._maps_padded.dtype).to(device)
    mask = torch.ones((divs**2,3,int(mask_w/divs),int(mask_h/divs)), device=device)
    ...
    delta_1 =torch.nn.Parameter( torch.zeros((mask.shape[0],3,1,1),requires_grad=True, device=device))
    ones_mask = torch.ones_like(meshes.textures.maps_padded())
    ...
    delta_1_masks.retain_grad()
    2
    meshes.textures._maps_padded =meshes.out_sizetextures.maps_padded()*ones_mask*delta_1_masks
#%%
...
#%%
# RGB images
# image_grid(images.detach().cpu().numpy(), rows=2, cols=5, rgb=True)
# plt.show()
...
if 0:
    ...
    # image = images[...,:3].clone()
    
    img =  (image*255).to(torch.uint8).cpu().detach().numpy()
    im = np.copy(img)
    img = im
    
    data = dict(img=img,
                gt_bboxes=bbox[image_idx],
                gt_labels=labels[image_idx],
    gt_bboxes_ignore=bboxes_ignore[image_idx] )
    cfg_test = cfg.copy()
    # set loading pipeline type
    
    cfg_test.test_pytorch3d_pipeline[0].type= 'LoadImageFromWebcam'
    
    
    # build the data pipeline
    test_pipeline = Compose(cfg_test.test_pytorch3d_pipeline)
...
# # prb = result[4][0][4]; print(prb)

# model.CLASSES=cfg.classes
# # show the resultsvis/exp2_12-2019/14_Day_R_1600_H_800_pose_C/
# show_result_pyplot(model, img, result, score_thr=0.2)

# img = img + (np.random.randn(*img.shape)*10).astype(int)
